...
import editInterest from "./Edit/formEdit"
...
const domAppender = {
    transformData(s) {
        ...
        $("<button>").attr({"id": "edit-btn", "type": "submit"}).text("Edit").click((event) => {event.preventDefault(); editInterest.handleEdit()}).appendTo(displayCardsContainer)
        $("<button>").attr({"id": "delete-btn", "type": "submit"}).text("Delete").click((event) => {event.preventDefault(); editInterest.handleDelete()}).appendTo(displayCardsContainer)
        $("<button>").attr({"id": "save-btn", "type": "submit"}).text("Save").click((event) => {event.preventDefault(); editInterest.handleSave()}).appendTo(displayCardsContainer)
